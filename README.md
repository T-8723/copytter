...
アプリケーションの基本構成（Google Cloud Platform へのデプロイ、Cloud Platform でのサービス有効化、フロント・バックエンドの環境構築、バックエンドのＤＢ連携）は一通り完了して、実際に[Google上のページ](https://copytter.an.r.appspot.com/)でアクセス可能な状態です。  
